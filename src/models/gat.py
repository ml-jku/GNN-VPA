...
class GATModel(pl.LightningModule):
    def __init__(self, num_layers, num_features, conv_hidden_dim, fc_hidden_dim, num_classes,
        ...
        for layer in range(num_layers):
            if layer == 0:
                conv = GATConv(self.num_features, self.conv_hidden_dim, aggr=self.neighbor_pooling_type)
            else:
                conv = GATConv(self.conv_hidden_dim, self.conv_hidden_dim, aggr=self.neighbor_pooling_type)
